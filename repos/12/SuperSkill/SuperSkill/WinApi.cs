...
namespace Transform
{
    ...
    public class TransCtr
    {
        ...
        public static byte[] AnsiToUnicode(string Ansi)
        {
            if (Ansi == "")
                return BitConverter.GetBytes(0);
            byte[] 数据2 = { };
            int 长度;
            长度 = MultiByteToWideChar( 0, 0, Ansi, Ansi.Length, 数据2, 0);
            byte[] 数据1 = new byte[长度 * 2];
            MultiByteToWideChar(0, 0, Ansi, Ansi.Length, 数据1, 0);
            return 数据1;
        }
