...
namespace SuperSkill
{
    public partial class Form1 : Form
    {
        ...
        private void ListView_Skill_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            ...
            技能CD = EncDec.Decrypt(全局变量.进程ID, (uint)ReadWriteCtr.ReadMemInt(全局变量.进程ID, (uint)技能地址1 + 基址.技能冷却7) + 8*(uint)(技能等级-1), 基址.解密基址) / 1000;
            ...
            while (i1 < 13)
            {
                ...
                if (总技能公式.IndexOf(Convert.ToString(技能地址2)) == -1 && 技能地址2 > 1000000000)
                {
                    ...
                    if (i1 == 0)
                        MessageBox.Show(Convert.ToString(技能数据), "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    if (技能数据 > 1)
                    {
